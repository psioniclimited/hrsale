function update_total() { 
	...
	var tax_figure = $('.tax_figure').val();
	//var tax_rate = $('.tax_rate').val();
	if($('.tax_type').val() == '1'){
		 var fxsub_total = tax_figure;
		  //var discount_amval = discount_figure;//.toFixed(2);
		  $('.tax_amount').val(tax_figure);
		  //$('.grand_total').html(grand_total.toFixed(2));	 
	 } else {
		 var tax_percent = sub_total / 100 * tax_figure;
		 var fxsub_total = tax_percent;
		// var discount_amval = discount_percent.toFixed(2);
		$('.tax_amount').val(tax_percent.toFixed(2));
		 //$('.grand_total').html(grand_total.toFixed(2));	 
	 }
	var xitem_sub_total = parseFloat(sub_total) + parseFloat(fxsub_total);
	var item_sub_total = parseFloat(sub_total);// + parseFloat(fxsub_total);
	...
	//var sub_total = $('.items-sub-total').val();
	 ...
	 if($('.discount_type').val() == '1'){
		 var fsub_total = xitem_sub_total - discount_figure;
		  //var discount_amval = discount_figure;//.toFixed(2);
		  $('.discount_amount').val(discount_figure);
		  //$('.grand_total').html(grand_total.toFixed(2));	 
	 } else {
		 var discount_percent = xitem_sub_total / 100 * discount_figure;
		 var fsub_total = xitem_sub_total - discount_percent;
		// var discount_amval = discount_percent.toFixed(2);
 ...
 jQuery(document).on('click keyup change','.qty_hrs,.unit_price',function() {
	...
	// var tax_rate = $(this).closest('.item-row').find('.tax_type').val();
	 ...
	 var sub_total = sbT.toFixed(2);	 
	jQuery(this).closest('.item-row').find('.sub-total-item').val(sub_total); 
	jQuery(this).closest('.item-row').find('.sub-total-item').val(sub_total);
	update_total();
});
jQuery(document).on('change click','.tax_typex', function () {
	var qty = 0;
	 var unit_price = 0;
	 var tax_rate = 0;
	 var qty = $(this).closest('.item-row').find('.qty_hrs').val();
...
jQuery(document).on('click keyup change','.tax_figure',function() {
	 var qty = 0;
	 var unit_price = 0;
	 var tax_rate = 0;
	 var tax_figure = $('.tax_figure').val();
	...
	if(parseFloat(tax_figure) <= parseFloat(sub_total)) {
	 if($('.tax_type').val() == '1'){
		 var grand_total = parseFloat(sub_total) + parseFloat(tax_figure);
		  var tax_amval = tax_figure;//.toFixed(2);
		  $('.tax_amount').val(tax_amval);
		  $('.grand_total').html(grand_total.toFixed(2));	 
	 } else {
		 var tax_percent = sub_total / 100 * tax_figure;
		 var grand_total = parseFloat(sub_total) + parseFloat(tax_percent);
		 var tax_amval = tax_percent.toFixed(2);
		 $('.tax_amount').val(tax_amval);
		 $('.grand_total').html(grand_total.toFixed(2));	 
	 }
	} else {
...
$(document).ready(function(){	
	...
	jQuery("#aj_company").change(function(){
		...
		jQuery.get(base_url+"/get_co_employees/"+jQuery(this).val(), function(data, status){
