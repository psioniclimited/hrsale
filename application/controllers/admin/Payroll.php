          ...
          foreach($payslip->result() as $r) {
			...
			foreach ($result->result() as $hour_work){
				...
				if($r->wages_type==1){
					...
					$wages_type = $this->lang->line('xin_employee_semi_monthly_payslip');
					$basic_salary = $r->basic_salary;
					$p_class = 'emo_semi_monthly_pay';
					$view_p_class = 'semi_template_modal';
				...
				if($count_allowances > 0) {
					foreach($salary_allowances as $sl_allowances){
						/*if($sl_allowances->is_allowance_taxable==0){
							...
							//$aw1 = $sl_allowances->allowance_amount/100 * 20;
							$allowance_amount -= $sl_allowances->allowance_amount;
						}*/
						$allowance_amount += $sl_allowances->allowance_amount;
					}
	...
	// payment history
	 ...
	 public function pdf_create(){
		...
		<table cellpadding="1" cellspacing="1" border="0">
			...
			<tr>
				<td align="center">'.$this->lang->line('xin_payroll_year_date').': <strong>'.date("F Y", strtotime($payment[0]->year_to_date)).'</strong></td>
			</tr>
		</table>
		';
